...
class Model_support extends CI_Model {
    ...
    public function __construct() {
        parent::__construct();
        $this->main_table = "admin_master";
        $this->primary_key = "iAdminId";
    }

    public function authenticate($email, $password) {          
        $ext = 'vPassword = "' . $password . '" AND  vEmail = "' . $email. '"';
        $this->db->select('iAdminId,vFirstName,vLastName,vImage');
        ...
        if (is_array($record) && count($record) > 0) {
            if ($record[0]['eStatus'] == 'Inactive') {
                ...
                $datalog['iUserId'] = $record[0]['iAdminId'];
                $datalog['vSessionId'] = $this->general->encryptData($email . 'Admin' . date('Y-m-d h:i:s'));
//                $datalog['vIP'] = $this->input->ip_address();
                $datalog['eUserType'] = 'Admin';
    ...
    function createActivityMessage($tbl, $idField, $id, $postArray) {
        ...
        foreach ($result1 as $key => $value) {
            ...
            if (array_key_exists($key, $postArray) && !in_array($key, $exclude_fields)) {
                ...
                //echo $key . "===>" . $value . "----------" . $postArray[$key] . "<br/>";
                //var_dump($postArray[$key] != $value);
                // var_dump(($postArray[$key] != '' && $value != ''));
                // var_dump(($value != '0000-00-00' && $postArray[$key] != '-0001-11-30'));
...
//                    if (trim($postArray[$key]) != '' && trim($postArray[$key]) != '0000-00-00' && trim($postArray[$key]) != '0') {
...
//                    }
                    ...
                    if (array_key_exists($key, $referenceField)) {
                        // echo $key . "---" . $fromvalue . "---" . $tovalue . "---" . "<br/>";

                        $referenceFieldArray = explode(":", $referenceField[$key]);

                        // pr($referenceFieldArray);
                        ...
                        if ($fromtable == "dynamictable") {
                            ...
                            //if (strtolower($type) != 'none' || $type != '0' || $type != '') {
                            // echo "in if" . $type;
                            //   pr($otherField);
                            //   pr($otherField['Lead']);
                            //  echo   $otherField[$type]; // lead_master:iLeadId:vTitle
                            $referenceFieldArrayFrom = explode(":", $otherField[$fromtype]);
                            $referenceFieldArrayTo = explode(":", $otherField[$totype]);
                            //    pr($referenceFieldArrayFrom);
                            //     pr($referenceFieldArrayTo);
                            ...
                            //$value = 0;
                            //  echo "SELECT " . $fromfetchField . " FROM " . $fromtable . " WHERE " . $fromfetchId . " = " . $value;
                            //  echo "<hr>SELECT " . $tofetchField . " FROM " . $totable . " WHERE " . $tofetchId . " = " . $postArray[$key];
                        }
...
//                    $d = DateTime::createFromFormat('Y-m-d', $tovalue);
//                    $isdate_post = $d && $d->format('Y-m-d') == $tovalue;
//
//                    $d1 = DateTime::createFromFormat('Y-m-d', $fromvalue);
//                    $isdate_db = $d1 && $d1->format('Y-m-d') == $fromvalue;
//
//                    if ($isdate_post) {
//                        $tovalue = $this->general->changeDateTimeFormat();
//                    }
//
//                    if ($isdate_db) {
//                        $fromvalue = $this->general->changeDateTimeFormat();
//                    }
    ...
    /*   function insertUpdateMessage1($tbl, $id, $postArray) {

      $callToField = array('Lead' => 'lead_master:iLeadId', 'Contact' => 'contact_master:iContactId');

      $relatedToField = array('Account' => 'account_master:iAccountId', 'Vendor' => 'vendor_master:iVendorId', 'Case' => 'case_master:iCaseId');
      ...
      $addActivity['iUserId'] = $this->session->userdata('iUserId');
      $addActivity['eItemUrl'] = '';
      $addActivity['tMessage'] = $message;
      $addActivity['tChange'] = 'updated_message';
      $addActivity['dtCreated'] = date('Y-m-d H:i:s');
      ...
      $fields = "iActivityId, count(*) as count";
      $ext_cond = 'tChange = "updated_message" AND eItemType ="User" AND iItemId = ' . $postArray['iAssignedId'] . ' AND DATE(dtCreated) = "' . date('Y-m-d') . '"';
      $reply = $this->getData('activity_master', $fields, array(), $ext_cond);
      ...
      $fields = "iActivityId, count(*) as count";
      $ext_cond = 'tChange = "updated_message" AND eItemType ="' . $callTo . '" AND iItemId = ' . $callToId . ' AND DATE(dtCreated) = "' . date('Y-m-d') . '"';
      $reply = $this->getData('activity_master', $fields, array(), $ext_cond);
      ...
      $fields = "iActivityId, count(*) as count";
      $ext_cond = 'tChange = "updated_message" AND eItemType ="' . $relatedTo . '" AND iItemId = ' . $relatedToId . ' AND DATE(dtCreated) = "' . date('Y-m-d') . '"';
      $reply = $this->getData('activity_master', $fields, array(), $ext_cond);
