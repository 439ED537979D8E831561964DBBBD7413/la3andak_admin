...
class V1 extends MX_Controller {
            ...
            if (!empty($seller_id) && !empty($sub_cat_id) && $brand_flag == 0) {
                ...
                if ($sub_cat) {
                    ...
                    for ($i = 0; $i < count($result); $i++) {
                        ...
                        $sub_product_sql = $this->db->query("SELECT  p.product_id as subproduct_id, p.subproduct_id as main_product_id,p.discounted_price as product_price,p.mrp,p.quantity,p.unit_value ,u.display_name  FROM product AS p  LEFT JOIN unit_type u ON  (p.unit_type=u.unit_id) WHERE  p.subproduct_id=" . $product_data['product_id']);
    ...
    public function get_deleivery_charges() {
    ...
    public function order_add() {
        ...
        /* $data =  array();
          $msg = "LA3ANDAK WILL BE CLOSED DURING EID AND WILL BE RESUMING OUR SERVICE ON SUNDAY THE 17TH
          ENJOY THE HOLIDAY";
          $success = false;
          $this->json_code($msg,$data,$success);
          return; */
        ...
        if (time() > strtotime('22:00:00') || time() < strtotime('9:00:00')) {
            if ($flag == 1) {
                $this->break_order();
            }
        }
...
// send  invoice to user
        $sendmail1 = mail($to, $subject, $message, 'MIME-Version: 1.0' . "\r\n" . 'Content-type: text/html; charset=iso-8859-1' . "\r\n" . 'From: LA3ANDAK <support@la3andak.com>' . "\r\n");

// send the same invoice to admin
        // $sendmail1=mail("jadkaakani1@gmail.com", "New Order Is Placed By User", $message, 'MIME-Version: 1.0'."\r\n".'Content-type: text/html; charset=iso-8859-1'."\r\n".'From: LA3ANDAK <support@la3andak.com>' . "\r\n");
