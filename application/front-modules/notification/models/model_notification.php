...
class Model_notification extends CI_Model {
    ...
    public function __construct() {
        parent::__construct();
        $this->main_table = "product_master";
        $this->primary_key = "iProductId";
    }
     public function ios_push($num,$msg,$msg1,$im,$type)
      {
        $field="*";
        $user = $this->model_notification->getData("users", "*","bIsdelete=0 and ustatus=1 and vmod = 2 and isNotification = 1");
        $num=count($user);
       // var_dump($user);die();
        ...
        stream_context_set_option($ctx, 'ssl', 'local_cert', $_SERVER["DOCUMENT_ROOT"].'/grocery/application/front-modules/notification/models/certi_distri.pem');
        ...
        $message1=array('title'=>'','type'=>"1",'body'=>$msg);
        $body['aps'] = array(
            'alert' =>array('title'=>'' ,'type'=>"1",'body'=> $msg),
            'message' => $msg,
            'sound' => 'default.mp3',
            'mutable-content'=>1,
            'content-available'=>1,
        ...
        $body['aps'] = array(
            'alert' =>array('title'=>'' ,'type'=>"1",'body'=> $msg1),
            'message' => $msg1,
            'sound' => 'default.mp3',
            'mutable-content'=>1,
            'content-available'=>1,
   ...
   //   var_dump($result);die();
    ...
    public function send($num,$msg,$msg1,$im,$type)
    {
        ...
        for($j=0;$j<=$i;$j++)
        {
            ...
            {
                array_push($idarray, $row->fcm_id);
                array_push($user_ids,$row->user_id);
            }

             $url = 'https://fcm.googleapis.com/fcm/send';
             $message=array('message'=>$msg,'type'=>1,'image'=>$im);
        ...
        $message=array('message'=>$msg1,'type'=>1,'image'=>$im);
